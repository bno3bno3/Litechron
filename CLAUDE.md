...
- Foreground and background refresh are further serialized through `RefreshLock` (`lib/worker/refresh_lock.dart`), a timestamp lock in secure storage: the background task skips its round while the foreground lock is held, and the foreground waits up to 45 s for a running background refresh. `RefreshLock.enabled` is only set in `main()`; tests run with it off so refresh still starts synchronously. `Zdbk._withAutoRelogin` backs off before the second and later relogins (`Zdbk.reloginBackoff`) and gives the timetable more attempts than other endpoints; the user-facing "会话已过期且自动重登失败" message stays short on purpose (no retry trace). Spider-level `login()` reuses a successful result for `UgrsSpider.loginReuseWindow` to stop cascading relogins.
